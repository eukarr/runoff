...
# Введение  
Возможность заранее прогнозировать объем речного стока позволит существенно снизить затраты и повысить эффективность работы хозяйственных и природоохранных объектов. Одной из возможностей предсказать речной сток является анализ информации об осадках в зимний период. Интуитивно очевидно, что такая связь должна быть, но не менее важно (помимо подтверждения ее наличия) оценить силу этой связи и ее прогностические возможности.  
Таким образом, нулевая гипотеза, рассматриваемая в этом анализе, была сформулирована так: объем речного стока не зависит от величины осадков в той же местности в зимний период. Далее, в случае отказа от нее, то есть подтверждения наличия связи, предполагается построить и оптимизировать линейную модель этой связи на основе экспериментальных данных, определить ее статистические характеристики и использовать ее для предсказаний.  
...
Данные для анализа были загружены из пакета alr4 1.0.6 [1] (датасет water). В ходе анализа были использованы пакеты tidyverse 1.2.1 [2] (набор пакетов для основных операций по преобразованию и визуализации данных, который включает, в частности, пакеты ggplot2 3.2.1 [3], dplyr 0.8.3 [4] и tidyr 0.3.3 [5]), cowplot 1.0.0 [6] (организация нескольких графиков ggplot2 на рисунке), qqplotr 0.0.3 [7] (расширение ggplot2; для построения квантильных графиков), DAAG 1.22.1 [8] (для проведения кросс-валидации), GGally 1.4.0 [9] (расширение ggplot2; для попарного сравнения наборов данных) и knitr 1.24 [10] (оформление таблицы в отчете).  

Анализ состоял из следующих основных шагов (детали их реализации приведены в разделе "Расчеты", а полученные результаты комментированы в разделе "Результаты и обсуждение"):  
...
В этом разделе будет приведен и прокомментирован полный код скрипта, использованного для анализа, а его консольный и графический вывод будет приведен и проанализирован в разделе Результаты и обсуждение. Для удобства понимания при обсуждении результатов частично приводится код, формирующий соответствующий вывод.
...
old_dir <- getwd()
setwd("C:/Users/Evgeny/Dropbox/Coursera/RStat2019")
